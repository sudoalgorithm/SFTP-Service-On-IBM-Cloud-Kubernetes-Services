...
if [ "$UPDATE" == "TRUE" ]; then
    ...
    cp -r $HELM_HOME/cache/plugins/ibmcloud-object-storage-plugin/helm-ibmc $HELM_HOME/cache/plugins/
    rm -rf $HELM_HOME/cache/plugins/ibmcloud-object-storage-plugin
    PLUGIN_LOCATION=$(ls -l $HELM_PLUGIN_DIR | awk -F ' -> ' '{print $2}')
    if [[ "$PLUGIN_LOCATION" != "$HELM_HOME/cache/plugins/helm-ibmc" ]]; then
      rm -rf $HELM_PLUGIN_DIR
      ln -s $HELM_HOME/cache/plugins/helm-ibmc $HELM_PLUGIN_DIR
...
if [ "$COMMAND" == "install" ]; then
    ...
    echo "DC: ${DC_NAME}  Chart: $CHARTNAME"
    ...
    helm install --set dcname="${DC_NAME}" "$CHARTNAME" "${FLAGS[@]}"
    if [[ $flag -eq 1 ]]; then
      if [[ "$CHARTNAME" = *"/"* ]] && [[ "$CHARTNAME" != ./* ]]; then
  ...
  helm upgrade --set dcname="${DC_NAME}" "$RELEASE_NAME" "$CHARTNAME" "${FLAGS[@]}"
  if [[ $flag -eq 1 ]]; then
    if [[ "$CHARTNAME" = *"/"* ]] && [[ "$CHARTNAME" != ./* ]]; then
      rm -rf $(echo "$CHARTNAME" | awk -F '/' '{print $2}')
    elif [[ "$CHARTNAME" = *".tgz"* ]]; then
